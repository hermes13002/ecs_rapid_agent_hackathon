...
def test_run_simulation(mock_rawread_class, mock_simrunner_class, mock_generate_netlist):
    """
    Test the entirely in-memory run_simulation function.
    Uses side_effects to write a dummy CSV into the dynamic temporary directory.
    """
    # 1. Setup Data
    mock_schematic = MagicMock()
    mock_generate_netlist.return_value = "* Mocked Netlist\nV1 1 0 DC 5 AC 1\n.op\n.end"

    # 2. Setup SimRunner Mock
    mock_runner_instance = MagicMock()
    mock_simrunner_class.return_value = mock_runner_instance
    # Return dummy strings for the raw and log file paths
    mock_runner_instance.run_now.return_value = ("dummy_circuit.raw", "dummy_circuit.log")

    # 3. Setup RawRead Mock and the Side Effect Trick
    mock_rawread_instance = MagicMock()
    mock_rawread_class.return_value = mock_rawread_instance

    # This custom function will run when `out.to_csv()` is called in your code.
    # It takes the dynamic temp path your code generated and writes a real CSV to it!
    def fake_to_csv(target_csv_path):
        target_csv_path = Path(target_csv_path)
        target_csv_path.write_text("time,v(1)\n0.0,5.0\n0.1,5.0\n", encoding="utf-8")

    mock_rawread_instance.to_csv.side_effect = fake_to_csv

    # 4. Execute Function
    result = run_simulation(mock_schematic)

    # 5. Assertions

    # Verify the JSON output exactly matches our fake CSV data
    expected_data = [
        {"time": "0.0", "v(1)": "5.0"},
        {"time": "0.1", "v(1)": "5.0"}
    ]
    ...
    # Verify our external dependencies were called correctly
    mock_generate_netlist.assert_called_once_with(mock_schematic)
    mock_simrunner_class.assert_called_once()
    mock_runner_instance.run_now.assert_called_once()
    mock_rawread_class.assert_called_once_with("dummy_circuit.raw")
    mock_rawread_instance.to_csv.assert_called_once()
